...
class VeiculoController extends Controller
{
    ...
    public static function save()
    {
        ...
        $veiculo->revisao = array_key_exists('revisao', $_POST) ? 1 : 0; // Verificando se a checkbox foi selecionada ou não
        $veiculo->sinistro = array_key_exists('sinistro', $_POST) ? 1 : 0; // Verificando se a checkbox foi selecionada ou não
        $veiculo->roubo_furto = array_key_exists('roubo_furto', $_POST) ? 1 : 0; // Verificando se a checkbox foi selecionada ou não
        $veiculo->aluguel = array_key_exists('aluguel', $_POST) ? 1 : 0; // Verificando se a checkbox foi selecionada ou não
        $veiculo->venda = array_key_exists('venda', $_POST) ? 1 : 0; // Verificando se a checkbox foi selecionada ou não
        $veiculo->particular = array_key_exists('particular', $_POST) ? 1 : 0; // Verificando se a checkbox foi selecionada ou não
